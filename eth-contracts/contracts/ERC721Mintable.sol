...
//  5) create a Paused & Unpaused event that emits the address that triggered the event
    event Paused(address accnt);
    event Unpaused(address accnt);
}
contract ERC165 {
...
contract ERC721 is Pausable, ERC165 {
    ...
    function balanceOf(address owner) public view returns (uint256) {
        ...
        require(owner != address(0), "Owner does not exists");
        return _ownedTokensCount[owner].current();
    }

    function ownerOf(uint256 tokenId) public view returns (address) {
        // TODO return the owner of the given tokenId
        address tmp = _tokenOwner[tokenId];
        require (tmp != address(0));
        return tmp;
    }
...
//    @dev Approves another address to transfer the given token ID
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(to != address(0), "Address does not exists");
...
contract CustomERC721Token is ERC721Metadata("Decentralized Housing Product", "DCT", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") {
